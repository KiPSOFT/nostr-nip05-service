...
export class DB {
  ...
  async connect() {
    const mongoDBUri = `mongodb+srv://${Deno.env.get(
      "MONGODB_USER"
    )}:${Deno.env.get("MONGODB_PASSWORD")}@${Deno.env.get(
      "MONGODB_HOST"
    )}/nip05?authMechanism=SCRAM-SHA-1`;
  ...
  async getNonVerifiedUsers(): Promise<Array<User>> {
    let search: any = {
      verified: false,
      createdAt: {
        $gte: moment(new Date()).subtract(1, 'months')
      }
    };
